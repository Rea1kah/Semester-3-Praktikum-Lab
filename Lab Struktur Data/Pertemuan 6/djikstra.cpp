...
void dijkstra(int V, vector<vector<iPair>> &adj, int src) {
    // Menyimpan jarak terpendek dari src ke setiap vertex
    vector<int> dist(V, INT_MAX);
    priority_queue<iPair, vector<iPair>, greater<iPair>> pq;

    dist[src] = 0;
    pq.push({0, src}); // (jarak, vertex)
    ...
    while (!pq.empty()) {
        ...
        // Jika jarak yang diambil dari priority queue lebih besar dari jarak yang diketahui, lewati
        if (d > dist[u]) 
        {
            continue;
        }
        ...
        for(auto edge : adj[u])
        {
            ...
            // jika jarak melalui u lebih pendek, perbarui jarak ke v
            if(dist[u] + w < dist[v])
            {
                dist[v] = dist[u] + w;
                pq.push({dist[v], v});
        ...
        // Periksa semua tetangga vertex u
        for (auto &neighbor : adj[u]) 
        {
            int v = neighbor.first;
            int weight = neighbor.second;

            // Jika jarak baru lebih pendek, perbarui jarak dan masukkan ke priority queue
